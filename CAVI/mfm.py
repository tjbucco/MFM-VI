...
class CAVI_MFM:
    ...
    """
    def __init__(self, Y, T, a0=1.0, b0=0.3, lambda0=None, nu0=None, b0_mu=None, B0=None, alpha_lambda_prior=None, alpha_pi_prior=None, beta_pi_prior=None):
        """
        ...
        self.lambda0 = 0.5 if lambda0 is None else lambda0
        self.b0_mu = np.mean(Y, axis=0) if b0_mu is None else b0_mu
        self.nu0 = self.p + 4 if nu0 is None else nu0
        if B0 is None:
        ...
        self.alpha_lambda_prior = 1 if alpha_lambda_prior is None else alpha_lambda_prior
        self.alpha_pi_prior = 4 if alpha_pi_prior is None else alpha_pi_prior
        self.beta_pi_prior = 3 if beta_pi_prior is None else beta_pi_prior
        ...
        # Storage for the log-normalizing-constant term needed by _update_q_K
        # (sum_n log sum_j exp(log_rho_nj^{[K]})), populated by _update_q_S
        self.log_evidence_per_kappa = np.zeros(self.T)
    ...
    def _update_q_S(self):
            ...
            for k in range(self.T):
                Y_centered = self.Y - self.E_mu_k[k]
                quadratic_term = np.sum((Y_centered @ self.E_Omega_k_inv[k]) * Y_centered, axis=1)
            ...
            for kappa in range(1, self.T + 1):
                ...
                # log-normalizer of the UNnormalized log responsibilities: this is
                # sum_n log sum_j exp(log_rho_nj^{[kappa]}), the data-fit term
                # required by the manuscript's log-kappa expression
                log_norm = logsumexp(log_rho_nk, axis=1, keepdims=True)  # N x 1
                self.log_evidence_per_kappa[kappa-1] = np.sum(log_norm)

                log_r_nk = log_rho_nk - log_norm
    ...
    def _update_q_K(self):
            """
            Update variational pmf of the number of components K, following the
            manuscript's exact expression for log(tilde kappa_K):
            """
            
            E_alpha_eta = self.a_eta_hat / self.b_eta_hat
            if E_alpha_eta <= 0:
                E_alpha_eta = 1e-6  # Safeguard

            # E_q[log alpha_eta] under Gamma(a_eta_hat, rate=b_eta_hat)
            E_log_alpha_eta = psi(self.a_eta_hat) - np.log(self.b_eta_hat)

            log_tilde_kappa = np.zeros(self.T)

            for K in range(1, self.T + 1):
                ratio = E_alpha_eta / K  # E_q[alpha_eta] / K

                # sum_{k=1}^{K} E_q[log eta_k^{[K]}]
                alpha_eta_k_hat_padded = 0.0001 * np.ones(self.T)
                alpha_eta_k_hat_padded[:len(self.alpha_eta_k_hat[K-1])] = self.alpha_eta_k_hat[K-1]
                E_log_eta_sum = np.sum(
                    psi(self.alpha_eta_k_hat[K-1]) - psi(np.sum(self.alpha_eta_k_hat[K-1]))
                    )
                #E_log_eta_sum = np.sum(
                #    psi(alpha_eta_k_hat_padded) - psi(np.sum(alpha_eta_k_hat_padded))
                #    )
                #print(f"Alpha_eta_k_hat : {alpha_eta_k_hat_padded}")
                #E_log_eta_sum = 0
                
                term_log_p_K   = self.log_kappa_prior[K-1]
                term_linear    = K * E_log_alpha_eta
                term_neg_KlogK = -K * np.log(K)
                term_gammaln   = -K * gammaln(ratio + 1)
                term_taylor    = -E_alpha_eta * psi(ratio + 1) * (E_log_alpha_eta - np.log(E_alpha_eta))
                term_dirichlet = (ratio - 1) * E_log_eta_sum
                term_data_fit  = self.log_evidence_per_kappa[K-1]

                log_tilde_kappa[K-1] = (term_log_p_K + term_linear + term_neg_KlogK
                                        + term_gammaln + term_taylor
                                        + term_dirichlet + term_data_fit)
                # log_tilde_kappa[K-1] = (term_log_p_K + term_linear + term_neg_KlogK
                #                         + term_gammaln + term_taylor
                #                         + term_data_fit)
